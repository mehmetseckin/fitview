...
const getUserAccessToken = async (userId)=>{
  const { data, error } = await supabase.from("user_fitbit").select("access_token, refresh_token, expires_at").eq("user_id", userId).single();
  if (error || !data) {
  ...
  if (new Date() >= new Date(expires_at)) {
    ...
    await supabase.from("user_fitbit").update({
      access_token: refreshedToken.access_token,
      refresh_token: refreshedToken.refresh_token,
      expires_at: new Date(Math.floor(Date.now() / 1000) + refreshedToken.expires_in).toISOString()
    }).eq("user_id", userId);
